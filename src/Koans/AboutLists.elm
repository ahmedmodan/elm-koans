...
testSuite =
  suite
    ...
    , test
        ...
        (assertEqual (List.foldl (\curr total -> curr ++ total) "a" [ "b", "c", "d" ]) "dcba")
    , test
        "foldr reduces a list from the right"
        (assertEqual (List.foldr (\curr total -> curr ++ total) "a" [ "b", "c", "d" ]) "bcda")
    , test
        "scanl reduces a list from the left, building a list of intermediate results"
        (assertEqual (List.scanl (\curr total -> curr ++ total) "a" [ "b", "c", "d" ]) [ "a", "ba", "cba", "dcba" ])
    ]
